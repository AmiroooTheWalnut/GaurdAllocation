...
drawSolution(vertices,faces,X,Y,terrainPoints,d,pi_v,height,isParallel);

function drawSolution(vertices,faces,X,Y,terrainPoints,d,pi_v,height,isParallel)
...
for i=1:numGaurds
    if selectedGaurds(i,1)~=0
        orig=[selectedGaurds(i,1),1,height];
        [visibleFaces,visibleVerticesSizes] = calcVisibility(orig,vertices,faces,X,Y,terrainPoints,isParallel);
        generatedX=[];
        generatedY=[];
        generatedZ=[];
        ...
        scatter3(selectedGaurds(i,1),1,height,'MarkerEdgeColor',colors(i,:),'MarkerFaceColor',colors(i,:))
...
%         for j=1:size(pi_v,2)
%             if selectedGaurds(i,1)==pi_v(1,j) || selectedGaurds(i,1)==pi_v(2,j)
%                 generatedX(1,size(generatedX,2)+1)=X(1,j);
%                 generatedX(1,size(generatedX,2)+1)=X(2,j);
%                 generatedY(1,size(generatedY,2)+1)=Y(1,j);
%                 generatedY(1,size(generatedY,2)+1)=Y(2,j);
%                 generatedZ(1,size(generatedZ,2)+1)=terrainPoints(1,j);
%                 generatedZ(1,size(generatedZ,2)+1)=terrainPoints(2,j);
%             end
%         end
%         scatter3(generatedX,generatedY,generatedZ,'MarkerEdgeColor',colors(i,:),'MarkerFaceColor',colors(i,:))
%         scatter3(X(1,selectedGaurds(i,1)),Y(1,selectedGaurds(i,1)),height,'MarkerEdgeColor',colors(i,:),'MarkerFaceColor',colors(i,:))
    end
end
view([45,45])
end
